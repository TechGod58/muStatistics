...
export function retrieveEvidence(params) {
    ...
    const filteredApplications = applications.filter((application) => {
        if (query.codeId && application.codeId !== query.codeId)
            return false;
        if (query.caseId && application.caseId !== query.caseId)
            return false;
        if (query.coderId && application.coderId !== query.coderId)
            return false;
        return true;
    });
    ...
    return segments
        .filter((segment) => {
        if (query.sourceId && segment.sourceId !== query.sourceId)
            return false;
        if (query.segmentKind && segment.kind !== query.segmentKind)
            return false;
        if (query.sourceKind && sourceMap.get(segment.sourceId)?.kind !== query.sourceKind)
            return false;
        if (query.codeId || query.caseId || query.coderId) {
            if (!applicationsBySegmentId.has(segment.id))
                return false;
        }
        if (query.coCodeId) {
        ...
        if (normalizedSearch && !segment.text.toLowerCase().includes(normalizedSearch))
            return false;
        if (query.memoOnly) {
            const hasMemo = memos.some((memo) => (memo.targetType === 'segment' && memo.targetId === segment.id)
        ...
        for (const application of segmentApplications) {
            if (application.caseId)
                relatedCaseIds.add(application.caseId);
        }
        ...
        return {
            ...
            cases: [...relatedCaseIds]
                ...
                .filter((caseEntity) => Boolean(caseEntity)),
            applications: segmentApplications,
            memos: memos.filter((memo) => (memo.targetType === 'segment' && memo.targetId === segment.id)
                || (memo.targetType === 'source' && memo.targetId === segment.sourceId))
        };
    })
...
function buildFilteredSegmentApplications(params) {
    ...
    return segments
        ...
        if (query.sourceKind && sourceMap.get(segment.sourceId)?.kind !== query.sourceKind)
            return false;
        const segmentApplications = applicationsBySegmentId.get(segment.id) ?? [];
        if ((query.caseId || query.coderId || query.codeId || query.coCodeId) && segmentApplications.length === 0)
            return false;
        if (query.codeId && !segmentApplications.some((application) => application.codeId === query.codeId))
            return false;
        if (query.coCodeId && !segmentApplications.some((application) => application.codeId === query.coCodeId))
            return false;
        if (normalizedSearch && !segment.text.toLowerCase().includes(normalizedSearch))
            return false;
        if (query.memoOnly) {
...
function tokenMatchesClause(value, operator, target) {
    ...
    if (operator === 'whole_word')
        return new RegExp(`\\b${escapeRegex(normalizedValue)}\\b`, 'i').test(target);
...
function matchCompoundClause(match, clause) {
    ...
    switch (clause.field) {
        ...
        case 'case':
            return match.cases.some((caseEntity) => tokenMatchesClause(value, clause.operator, caseEntity.id)
        ...
        case 'memo':
            if (clause.operator === 'present')
                return match.memos.length > 0;
            return match.memos.some((memo) => tokenMatchesClause(value, clause.operator, memo.title)
...
export function buildCodeHierarchy(params) {
    ...
    for (const application of params.applications) {
        if (!scopedSegmentIds.has(application.segmentId))
            continue;
...
export function buildConceptMap(params) {
    ...
    for (const relationship of params.relationships) {
        if (relationship.leftTargetType !== 'code' || relationship.rightTargetType !== 'code')
            continue;
    ...
    for (const item of items) {
        ...
        for (let leftIndex = 0; leftIndex < codeIds.length; leftIndex += 1) {
            for (let rightIndex = leftIndex + 1; rightIndex < codeIds.length; rightIndex += 1) {
                ...
                if (params.query.codeId && primaryCodeId !== params.query.codeId && secondaryCodeId !== params.query.codeId)
                    continue;
                if (params.query.coCodeId && primaryCodeId !== params.query.coCodeId && secondaryCodeId !== params.query.coCodeId)
                    continue;
...
export function buildFrameworkMatrix(params) {
    const summarizeFrameworkText = (value, maxLength) => {
