...
export type ForecastMethod = 'linear_trend' | 'moving_average' | 'exponential_smoothing';
...
export function analyzeForecast(
  ...
  const method: ForecastMethod = settings?.method ?? 'linear_trend';
  ...
  if (method === 'moving_average') {
    ...
    projectedValues = Array.from({ length: normalizedHorizon }, () => {
      ...
      while (trail.length > movingAverageWindow) trail.shift();
