...
function computeClassificationMetricsAtThreshold(probabilities, outcomes, weights, threshold) {
...
function buildCalibrationBins(probabilities, outcomes, weights, binCount = 10) {
...
function uniqueFormattedLevels(values) {
    return [...new Set(values
            .filter((value) => value !== null && value !== undefined)
            .map((value) => formatValue(value)))]
        .sort((left, right) => left.localeCompare(right));
}
export function analyzeGeneralLinearModel(dataset, dependentFieldKey, factorFields = [], covariateFields = [], options) {
    ...
    const rows = analysisRows(dataset, requiredFields, options)
        ...
        factorValues: Object.fromEntries(normalizedFactors.map((field) => [field, row[field] ?? null])),
        covariateValues: Object.fromEntries(normalizedCovariates.map((field) => [field, row[field] ?? null])),
        weight
    }))
...
export function analyzeRepeatedMeasures(dataset, fields, options) {
...
function isSurvivalEvent(value) {
    if (typeof value === 'boolean')
        return value;
    if (typeof value === 'number')
    ...
    if (typeof value === 'string') {
        ...
        if (!normalized)
            return false;
        if (['1', 'true', 'yes', 'y', 'event', 'dead', 'death', 'complete', 'completed', 'failed'].includes(normalized))
            return true;
        if (['0', 'false', 'no', 'n', 'censored', 'alive', 'withdrawn'].includes(normalized))
            return false;
    }
    return false;
}
export function analyzeSurvivalAnalysis(dataset, timeFieldKey, eventFieldKey, groupFieldKey, options) {
    ...
    const rows = analysisRows(dataset, requiredFields, options)
        ...
        .filter((entry) => entry.time !== null && Number.isFinite(entry.time) && entry.time >= 0 && Boolean(entry.groupValue));
    if (rows.length < 2) {
...
function splitSyntaxCommands(syntax) {
...
function cleanSyntaxToken(token) {
    return token.trim().replace(/^[("'`]+|[)"'`,]+$/g, '');
}
function resolveSyntaxField(dataset, token) {
...
function parseSyntaxFieldList(dataset, text) {
    const raw = text.trim();
    if (!raw)
        return [];
    if (/^ALL$/i.test(raw)) {
...
function extractSyntaxClause(command, clause) {
    const regex = new RegExp(`${clause}\\s*=\\s*([^/]+)`, 'i');
    const match = command.match(regex);
    return match?.[1]?.trim() ?? null;
}
function runSyntaxCommand(dataset, command, options) {
    ...
    try {
        if (commandName === 'DESCRIPTIVES' || commandName === 'DESCRIPTIVE') {
            ...
            if (fields.length === 0)
                throw new Error('DESCRIPTIVES requires at least one numeric field.');
        ...
        if (commandName === 'FREQUENCIES' || commandName === 'FREQUENCY') {
            ...
            const fields = parseSyntaxFieldList(dataset, variableClause);
            if (fields.length === 0)
                throw new Error('FREQUENCIES requires at least one field.');
            const report = describeDataset(dataset, options);
            return {
                command,
                commandName,
        ...
        if (commandName === 'CROSSTABS' || commandName === 'CROSSTAB') {
            ...
            if (!match)
                throw new Error('CROSSTABS requires /TABLES=rowField BY columnField.');
        ...
        if (commandName === 'CORRELATIONS' || commandName === 'CORRELATION') {
            ...
            const fields = parseSyntaxFieldList(dataset, variableClause)
                .filter((field) => dataset.fields.find((candidate) => candidate.key === field)?.valueType === 'number');
            if (fields.length < 2)
                throw new Error('CORRELATIONS requires at least two numeric fields.');
            const correlations = [];
        ...
        if (commandName === 'REGRESSION') {
            ...
            if (predictorFields.length === 0)
                throw new Error('REGRESSION requires at least one predictor field.');
            return {
                command,
                commandName,
                status: 'ok',
...
export function runSyntax(dataset, syntax, options) {
...
export function analyzeRegression(dataset, dependentField, predictorField, model, options) {
    ...
    if (!thresholdCandidates.includes(0.5))
        thresholdCandidates.push(0.5);
    ...
    const bestByF1 = [...thresholdAnalysis]
        ...
        .sort((left, right) => (right.f1Score - left.f1Score)
    ...
    const bestByYouden = [...thresholdAnalysis]
        ...
        .sort((left, right) => (right.youdenJ - left.youdenJ)
    ...
    const meanAbsoluteCalibrationError = totalCalibrationWeight > 0
        ...
        : null;
    const maxCalibrationGap = calibrationBins.reduce((max, bin) => {
        if (bin.calibrationGap === null)
            return max;
        const absoluteGap = Math.abs(bin.calibrationGap);
        return max === null ? absoluteGap : Math.max(max, absoluteGap);
    }, null);
    assumptions.push(buildAssumptionCheck('calibration', 'Calibration (mean absolute gap)', meanAbsoluteCalibrationError === null ? 'warn' : meanAbsoluteCalibrationError <= 0.1 ? 'pass' : 'warn', meanAbsoluteCalibrationError, meanAbsoluteCalibrationError === null
